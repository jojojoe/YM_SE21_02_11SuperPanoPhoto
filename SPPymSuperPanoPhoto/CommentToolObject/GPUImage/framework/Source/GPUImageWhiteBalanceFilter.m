...
void main()
{
    ...
    lowp vec4 finalColor = vec4(mix(rgb, processed, temperature), source.a);
    
    lowp float luminance = dot(finalColor.rgb, luminanceWeighting);
    lowp vec3 greyScaleColor = vec3(luminance);
    
    finalColor = vec4(mix(greyScaleColor, finalColor.rgb, saturation), finalColor.a);

	gl_FragColor = vec4((finalColor.rgb + vec3(brightness)), finalColor.a);
}
);
#else
NSString *const kGPUImageWhiteBalanceFragmentShaderString = SHADER_STRING
