...
#define ORIGINAL_AUDIO_TRACK_ID 200
#define MIX_AUDIO_TRACK_ID 201
...
- (id)initWithURL:(NSURL *)url;
{
    ...
    [inputAsset loadValuesAsynchronouslyForKeys:assetKeysToLoadAndTest completionHandler: ^{
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            ...
            //            blockSelf->previousFrameTime = kCMTimeZero;
...
- (void)startProcessing
{
    ...
    previousFrameTime = kCMTimeZero;
    previousActualFrameTime = CFAbsoluteTimeGetCurrent();
    _VO_currentFrameIndex = 0;
    if (self.asset) {
        [reader cancelReading];
        reader = nil;
        if (self.VO_originalMovie) {
            [self processAsset];
        } else {
            dispatch_async(dispatch_get_global_queue(0, 0), ^{
                [self processAsset];
...
- (AVAssetReader*)createAssetReader
{
    ...
    if (shouldRecordAudioTrack)
    {
        ...
        //
        //                                };
        readerAudioTrackOutput = [AVAssetReaderTrackOutput assetReaderTrackOutputWithTrack:audioTrack outputSettings:@{AVFormatIDKey:@(kAudioFormatLinearPCM)}];
...
- (void)processAsset
{
    ...
    if ([reader startReading] == NO)
    {
        ...
        NSLog(@"Error reading from file at URL: %@ and error:%@", self.url,[reader.error localizedDescription]);
        return;
    }
    
    if (_VO_originalMovie) {
        return;
    }
    ...
    if (synchronizedMovieWriter != nil)
    {
        [synchronizedMovieWriter setVideoInputReadyCallback:^{
            ...
            if (success) {
                if (weakSelf.VO_currentAddonMovie && [weakSelf respondsToSelector:@selector(VO_currentAddonMovie)]) {
                    AVAssetReaderOutput *addonTrackOutput = nil;
                    for( AVAssetReaderOutput *output in weakSelf.VO_currentAddonMovie.assetReader.outputs) {
                        if( [output.mediaType isEqualToString:AVMediaTypeVideo] ) {
                            addonTrackOutput = output;
                        }
                    }
                    [weakSelf.VO_currentAddonMovie readNextVideoFrameFromOutput:addonTrackOutput];
                }
            }
            return success;
        }];
...
- (void)setVO_pauseReading:(BOOL)VO_pauseReading {
    [displayLink setPaused:VO_pauseReading];
}
...
- (void)displayLinkCallback:(CADisplayLink *)sender
{
	...
	if ([playerItemOutput hasNewPixelBufferForItemTime:outputItemTime] || forceRefreshFrame) {
        ...
        if( pixelBuffer ) {
            ...
            self.VO_frameRate = CMTimeSubtract(outputItemTime, previousFrameTime);
            previousFrameTime = outputItemTime;
            
            if (weakSelf.VO_currentAddonMovie) {
                AVAssetReaderOutput *addonTrackOutput = nil;
                for( AVAssetReaderOutput *output in weakSelf.VO_currentAddonMovie.assetReader.outputs) {
                    if( [output.mediaType isEqualToString:AVMediaTypeVideo] ) {
                        addonTrackOutput = output;
...
- (BOOL)readNextVideoFrameFromOutput:(AVAssetReaderOutput *)readerVideoTrackOutput;
{
    if (reader.status == AVAssetReaderStatusReading && ! videoEncodingIsFinished )
    {
        if (self.VO_originalMovie) {
            ...
            NSInteger currentWatermarkIndex = roundf(self.VO_currentFrameIndex);
            CMTime currentSampleTime = kCMTimeZero;
            CMSampleBufferRef processingBuffer;
            if (self.VO_previousSampleBuffer) {
                currentSampleTime = CMSampleBufferGetOutputPresentationTimeStamp(self.VO_previousSampleBuffer);
            }
            NSInteger currentFrame = roundf(CMTimeGetSeconds(currentSampleTime) * track.nominalFrameRate);
            if (currentFrame > currentWatermarkIndex) {
                processingBuffer = self.VO_previousSampleBuffer;
            } else {
                if (self.VO_previousSampleBuffer) {
                    CMSampleBufferInvalidate(self.VO_previousSampleBuffer);
                    CFRelease(self.VO_previousSampleBuffer);
                    self.VO_previousSampleBuffer = nil;
                }
                processingBuffer = [readerVideoTrackOutput copyNextSampleBuffer];
                if (!processingBuffer) {
                    self.VO_previousSampleBuffer = nil;
                    self.VO_currentFrameIndex = 0;
                    [self startProcessing];
                    return YES;
                }
                self.VO_previousSampleBuffer = processingBuffer;
            }
            float step = 1;
            step = CMTimeGetSeconds(self.VO_originalMovie.VO_frameRate) * track.nominalFrameRate;
            if (step > 0) {
                NSInteger frameCount = roundf(track.nominalFrameRate * CMTimeGetSeconds(self.asset.duration));
                self.VO_currentFrameIndex += step;
            ...
            __unsafe_unretained GPUImageMovie *weakSelf = self;
            runSynchronouslyOnVideoProcessingQueue(^{
                [weakSelf processMovieFrame:processingBuffer];
            });
        ...
        if (sampleBufferRef)
        {
            ...
            CMTime currentSampleTime = CMSampleBufferGetOutputPresentationTimeStamp(sampleBufferRef);
            self.VO_frameRate = CMTimeSubtract(currentSampleTime, previousFrameTime);
            previousFrameTime = currentSampleTime;
//            if (self.VO_originalMovie) {
//                float step = 1;
////                if (CMTimeGetSeconds(self.VO_frameRate) != 0) {
//                    AVAssetTrack *track = [[self.VO_originalMovie.asset tracksWithMediaType:AVMediaTypeVideo] firstObject];
//                    step = 60 / track.nominalFrameRate;
//                    if (step > 1) {
//                        if (self.VO_counter % 2 != 0) {
...
- (void)VO_refreshFrame {
    forceRefreshFrame = YES;
}
