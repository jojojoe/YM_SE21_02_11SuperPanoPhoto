...
const mediump vec3 luminanceWeightingForSaturation = vec3(0.2125, 0.7154, 0.0721);
...
void main()
{
    ...
    lowp float luminanceSaturation = dot(result.rgb, luminanceWeightingForSaturation);
    lowp vec3 greyScaleColor = vec3(luminanceSaturation);
    
    lowp vec4 finalColor = vec4(mix(greyScaleColor, result.rgb, saturation), source.a);
